...
fit_linear <- cph(Surv(time, status) ~ age + sex, data = df)
fit_spline <- cph(Surv(time, status) ~ rcs(age, 3) + sex, data = df)
lrtest(fit_linear, fit_spline)  # 若P<0.05，选择样条模型

#(3) 方法三：图形诊断
# 绘制HR随age的变化
plot(Predict(fit_spline, age, fun = exp, ref.zero = TRUE)) +
...
#若违反，尝试样条
fit_spline <- cph(Surv(time, status) ~ rcs(age, 3))
cox.zph(fit_spline)
